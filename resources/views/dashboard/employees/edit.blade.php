            ...
            <div class="col-lg-8">
                <section class="panel">
                    <header class="panel-heading">
						<div class="row">
							<div class="col-sm-12">
								<a href="/dashboard/employees/{{$person->userid}}" class="btn btn-danger btn-sm">
									<i class="fa fa-chevron-left"></i>
		                            {{trans('_.Back')}}
		                        </a>
								<span class="ui-head">{{trans('employee.Edit Employee Information')}}</span>
							</div>
						</div>
                    </header>
                                ...
                                <div class="col-sm-9">
									<div class="checkboxes">
                                        <label class="label_check c_on">
											<input name="permission_product" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_product == '1') checked @endif> <span class="ui-icheck-text">{{trans('employee.Product management')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_enquiry" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_enquiry == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Jewel enquiry')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_tour" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_tour == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Package tours')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_book" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_book == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Tour booking')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_newsletter" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_newsletter == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Newsletter')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_news" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_news == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.News')}}</span>
										</label>
                                        <label class="label_check c_on hidden">
											<input name="permission_contact" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_contact == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Contacts')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_document" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_document == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Documents')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_payment" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_payment == '1') checked @endif> <span class="ui-icheck-text">{{trans('payment.Payments')}}</span>
										</label>
										<label class="label_check c_on">
											<input name="permission_setting" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_setting == '1') checked @endif> <span class="ui-icheck-text">{{trans('employee.The settings of the site')}}</span>
										</label>
										<label class="label_check c_on">
											<input name="permission_employee" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_employee == '1') checked @endif> <span class="ui-icheck-text">{{trans('employee.Employee management')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_gallery" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_gallery == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Gallery')}}</span>
										</label>
                                        <label class="label_check c_on">
											<input name="permission_video" value="true" type="checkbox" data-plugin="icheck" @if($person->permission_video == '1') checked @endif> <span class="ui-icheck-text">{{trans('_.Videos')}}</span>
										</label>
									</div>
								</div>
                            </div>
							<div class="form-group">
                                ...
                                <div class="col-sm-9">
									<label class="checkbox-inline" style="padding-left:0px;">
                                		<input type="radio" name="status" value="a" data-plugin="icheck" @if($person->status == 'a') checked @endif> {{trans('_.Active')}}
									</label>
									<label class="checkbox-inline">
                                		<input type="radio" name="status" value="b" data-plugin="icheck" @if($person->status == 'b') checked @endif> {{trans('_.Block')}}
									</label>
								</div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-offset-3 col-sm-9">
                                    <button type="submit" class="btn btn-danger" ng-disabled="passed">{{trans('_.Save changes')}}</button>
	...
	app.controller('FormController', function($scope, $http, $window){
		...
		$scope.checkUsername = function($event){
			...
			if ($scope.username && $scope.username.length > 4 && /^[a-zA-Z0-9_.]+$/.test($scope.username) && $scope.temp.username != $scope.username)
			{
				...
				$http.post('/ajax/dashboard/employees/{{$person->userid}}/exists/username', {
		...
		$scope.checkEmail = function($event){
			...
			if ($scope.email && /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i.test($scope.email) && $scope.temp.email != $scope.email)
			{
				...
				$http.post('/ajax/dashboard/employees/{{$person->userid}}/exists/email', {
					email: $scope.email
				})
				.success(function(resp){
					if (resp.status == 'ok')
