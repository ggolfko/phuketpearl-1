            ...
            <div class="col-lg-8">
                <section class="panel">
                    <header class="panel-heading">
						<div class="row">
							<div class="col-sm-12">
								<a href="/dashboard/docs/crown" class="btn btn-danger btn-sm">
									<i class="fa fa-chevron-left"></i>
		                            {{trans('_.Back')}}
		                        </a>
								<span class="ui-head">{{trans('crown.Edit Information')}}</span>
							</div>
						</div>
                    </header>
                        ...
                        <form class="form-horizontal tasi-form" method="post" action="{{$request->fullUrl()}}" name="form" id="form">
                                    ...
                                    <div>
                                        ...
                                        <div class="tab-content">
                                            ...
                                            <div role="tabpanel" class="tab-pane ui-tab-pane @if($index == 0) active @endif" id="title-{{$locale['code']}}">
                                                <input type="text" class="form-control" name="title[{{$locale['code']}}]" maxlength="128" autocomplete="off" data-input-title="{{$locale['code']}}" value="{{$crown->getTitle($locale['code'])}}">
                                    ...
                                    <div>
                                        ...
                                        <div class="tab-content">
                                            ...
                                            <div role="tabpanel" class="tab-pane ui-tab-pane @if($index == 0) active @endif" id="detail-{{$locale['code']}}">
                                                <textarea class="form-control" rows="4" style="resize:none;" data-plugin="autosize" name="detail[{{$locale['code']}}]" data-input-detail="{{$locale['code']}}">{!! $crown->getDetail($locale['code']) !!}</textarea>
                                                <p class="help-block"><em>{{trans('_.Please enter all information in each language.')}}</em></p>
                                            </div>
                                            @endforeach
                                        </div>
                            ...
                            <div class="form-group" data-group="image">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    @else
                                    <div id="img-preview">
                                        <img src="/app/crown/{{$crown->crownid}}/{{$crown->imageid}}.png" class="img-thumbnail">
                                    </div>
                                    @endif
								</div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-offset-3 col-sm-9">
                                    ...
                                    <input type="hidden" name="imageid" value="{{$crown->imageid}}" id="ipt-imageid">
        ...
        $('#uploadFile').bind('change', function(e){
            if ($(this).val() != '' && e.target.files.length == 1)
            {
                ...
                if ($.inArray(file.type.toLowerCase(), ['image/jpeg', 'image/gif', 'image/png']) >=  0)
                {
                    ...
                    $.ajax({
                        url: '/ajax/dashboard/docs/crown/image',
                        type: 'POST',
                        processData: false,
                        contentType: false,
                        dataType: "JSON",
                    ...
                    .success(function(resp){
                        if (resp.status == 'ok'){
                            ...
                            var img = $('<img />');
                            img.attr('src', '/app/crown/temp/'+resp.payload.imageid+'.png');
