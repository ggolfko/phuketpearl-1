            ...
            <div class="col-lg-12">
                <section class="panel" id="no-more-tables">
                    <header class="panel-heading">
                        <div class="row">
                            <div class="col-xs-12">
                                {{trans('_.Certificates')}}
                                <a href="/dashboard/docs/certificate/add" class="btn btn-sm btn-danger pull-right">
                                    {{trans('_.Add New')}}
                                </a>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3 ui-item" ng-controller="ItemController" ng-mouseenter="visible=true" ng-mouseleave="visible=false">
                                <div class="ui-action" ng-show="visible">
                                    <a href="/dashboard/docs/certificate/{{$item->certificateid}}" class="btn btn-xs btn-primary">{{trans('_.Edit')}}</a>
                                    <button type="button" class="btn btn-xs btn-danger" ng-click="delete({{$item->id}}, '{{$item->certificateid}}')" ng-disabled="deleting">{{trans('_.Delete')}}</button>
                                </div>
                                @if($item->imageid != '')
                                    <a href="/app/certificate/{{$item->certificateid}}/{{$item->imageid}}.png" class="fancybox">
                                        <img src="/app/certificate/{{$item->certificateid}}/{{$item->imageid}}_t.png" class="img-responsive">
...
(function(){
    ...
    app.controller('ItemController', function($scope, $http, $window){
        ...
        $scope.delete = function(id, certificateid){
            if (confirm('{{trans('_.Are you sure?')}}')){
                $scope.deleting = true;

                $http.post('/ajax/dashboard/docs/certificate/'+certificateid+'/delete', {
                    id: id
                })
