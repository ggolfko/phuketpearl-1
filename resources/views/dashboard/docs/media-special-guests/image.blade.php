                ...
                <section class="panel" id="no-more-tables">
                    <header class="panel-heading">
						<div class="row">
                            ...
                            <div class="col-md-7">
								<span class="ui-title">{{trans('_.Images')}}</span>
		                    </div>
                            <div class="col-md-4">
                    ...
                    <div class="panel-body" style="min-height:calc(100vh - 165px);">
                        <div class="row">
							...
							<div class="col-sm-4 ui-item" ng-controller="item" ng-class="{'hidden':deleted}" id="item-{{$image->imageid}}">
                                <a href="/app/media_special_guests/{{$item->itemid}}/{{$image->imageid}}.png" class="fancybox" rel="gallery">
                                    <img src="/app/media_special_guests/{{$item->itemid}}/{{$image->imageid}}_t.png" class="img-responsive">
                                </a>
                                <p class="ui-caption">
                                    <button type="button" class="btn btn-xs btn-danger pull-right" ng-disabled="deleting" ng-click="delete({{$image->id}},'{{$image->imageid}}')">{{trans('_.Delete')}}</button>
                                </p>
                            </div>
                            @endforeach
                        </div>
...
app.controller('item', function($scope, $http, $window){
    $scope.delete = function(id, imageid){
        ...
        {
            $scope.deleting = true;
            $http.post('/ajax/dashboard/docs/media-special-guests/{{$item->itemid}}/'+imageid, {
                id: id
            })
            ...
            if ($(this).val() != '' && e.target.files.length > 0)
            {
                ...
                if (valid)
                {
                    if (files.length <= 10)
                    {
                        ...
                        $.ajax({
                            url: '/ajax/dashboard/docs/media-special-guests/{{$item->itemid}}/upload',
                            type: 'POST',
                            processData: false,
                            contentType: false,
                            dataType: 'JSON',
                        ...
                        .success(function(resp){
                            if (resp.status == 'ok')
							{
								$http.post('/ajax/dashboard/docs/media-special-guests/{{$item->itemid}}/images', {
