                ...
                <section class="panel" id="no-more-tables">
                    <header class="panel-heading">
						<div class="row">
                            ...
                            <div class="col-md-7">
								<span class="ui-title">{{trans('timeline.Timeline Images')}}</span>
		                    </div>
                            <div class="col-md-4">
                    ...
                    <div class="panel-body" style="min-height:calc(100vh - 165px);">
                        <div class="row">
                            @foreach($timeline->images as $item)
                            <div class="col-sm-4 ui-item" ng-controller="ItemController" ng-class="{'hidden':deleted}" id="item-{{$item->imageid}}">
                                <a href="/app/timeline/{{$timeline->timelineid}}/{{$item->imageid}}.png" class="fancybox" rel="gallery">
                                    <img src="/app/timeline/{{$timeline->timelineid}}/{{$item->imageid}}_t.png" class="img-responsive">
                                </a>
                                <p class="ui-caption">
...
app.controller('ItemController', function($scope, $http, $window){
    $scope.delete = function(id, imageid){
        ...
        {
            $scope.deleting = true;
            $http.post('/ajax/dashboard/docs/timeline/{{$timeline->timelineid}}/'+imageid, {
                id: id
            })
            ...
            if ($(this).val() != '' && e.target.files.length > 0)
            {
                ...
                if (valid)
                {
                    if (files.length <= 10)
                    {
                        ...
                        $.ajax({
                            url: '/ajax/dashboard/docs/timeline/{{$timeline->timelineid}}/images',
                            type: 'POST',
                            processData: false,
                            contentType: false,
                            dataType: "JSON",
