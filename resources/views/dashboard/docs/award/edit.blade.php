            ...
            <div class="col-lg-8">
                <section class="panel">
                    <header class="panel-heading">
						<div class="row">
							<div class="col-sm-12">
								<a href="/dashboard/docs/award" class="btn btn-danger btn-sm">
                        ...
                        <form class="form-horizontal tasi-form" method="post" action="{{$request->fullUrl()}}" name="form" id="form">
                                    ...
                                    <div>
                                        ...
                                        <div class="tab-content">
                                            ...
                                            <div role="tabpanel" class="tab-pane ui-tab-pane @if($index == 0) active @endif" id="title-{{$locale['code']}}">
                                                <input type="text" class="form-control" name="title[{{$locale['code']}}]" maxlength="128" autocomplete="off" data-input-title="{{$locale['code']}}" value="{{$award->getTitle($locale['code'])}}">
                            ...
                            <div class="form-group" data-group="image">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    @else
                                    <div id="img-preview">
                                        <img src="/app/award/{{$award->awardid}}/{{$award->imageid}}.png" class="img-thumbnail">
                                    </div>
                                    @endif
								</div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-offset-3 col-sm-9">
                                    ...
                                    <input type="hidden" name="imageid" value="{{$award->imageid}}" id="ipt-imageid">
        ...
        $('#uploadFile').bind('change', function(e){
            if ($(this).val() != '' && e.target.files.length == 1)
            {
                ...
                if ($.inArray(file.type.toLowerCase(), ['image/jpeg', 'image/gif', 'image/png']) >=  0)
                {
                    ...
                    $.ajax({
                        url: '/ajax/dashboard/docs/award/image',
                        type: 'POST',
                        processData: false,
                        contentType: false,
                        dataType: "JSON",
                    ...
                    .success(function(resp){
                        if (resp.status == 'ok'){
                            ...
                            var img = $('<img />');
                            img.attr('src', '/app/award/temp/'+resp.payload.imageid+'.png');
