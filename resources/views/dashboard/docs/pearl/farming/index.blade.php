            ...
            <div class="col-lg-8">
                <section class="panel" id="no-more-tables">
                    <header class="panel-heading">
						<div class="row">
							<div class="col-md-3" style="padding-left:20px;">{{trans('pearl.Pearl Farming')}}</div>
							<div class="col-md-9">
								<form class="form-inline pull-right pull-left-sm" role="form" method="get" action="">
                                    <a href="/pearl-farming.html" target="_blank" class="btn btn-default btn-sm">
                                        {{trans('_.View on Frontend')}}
                                    </a>
                                    <a href="/dashboard/docs/pearlfarming/add" class="btn btn-danger btn-sm">
										{{trans('pearl.Add New')}}
									</a>
								</form>
							</div>
                        ...
                        <tbody>
							@if($items->count() > 0)
								@foreach($items as $index => $item)
									<tr ng-controller="Item" ng-init="id={{$item->id}};farmingid='{{$item->farmingid}}'">
										<td data-title="{{trans('_.Title')}}">{{$item->getTitle($config['lang']['code'])}}</td>
                                        <td class="text-right">
                                            <a href="/dashboard/docs/pearlfarming/{{$item->farmingid}}" class="btn btn-default btn-xs">{{trans('tour.View')}}</a>
                                            <a href="/dashboard/docs/pearlfarming/{{$item->farmingid}}/edit" class="btn btn-primary btn-xs">{{trans('_.Edit')}}</a>
    ...
    app.controller('Item', function($scope, $timeout, $http, $window){
        ...
        $scope.delete = function(){
            if (confirm('{{trans('_.Are you sure?')}}')){
                ...
                $http.post('/ajax/dashboard/docs/pearlfarming/'+$scope.farmingid+'/delete', {
                    id: $scope.id
                })
